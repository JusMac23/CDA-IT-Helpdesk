<x-app-layout>
    ...
    <style>
        ...
        @media (min-width: 640px) {
            ...
            /* Align Add User button and Search inline */
                                ...
                                <td>
                                    @if($user->roles->isNotEmpty())
                                        @foreach($user->roles as $role)
                                            <span style="display: inline-flex; align-items: center; white-space: nowrap; padding: 0.25rem 0.75rem; background-color: #eef2ff; color: #4338ca; border: 1px solid #c7d2fe; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; letter-spacing: 0.025em; text-transform: capitalize; margin: 0.125rem 0.25rem 0.125rem 0; box-shadow: 0 1px 2px rgba(0,0,0,0.02);">
                                                {{ $role->name }}
                                            </span>
                                        @endforeach
                                    @else
