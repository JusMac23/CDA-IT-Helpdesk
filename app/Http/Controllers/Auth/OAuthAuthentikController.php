    ...
    public function handleAuthentikCallback(Request $request)
    {
        try {
            ...
            if ($user->hasRole('Super Admin')) {
                ...
                \Log::info('Redirecting User to myrequested_tickets');
                return redirect()->route('myrequested_tickets.index');
            }
            elseif ($user->hasAnyRole(['Technical Personnel', 'Support Staff', 'Manager'])) {
                \Log::info('Redirecting other role to dashboard');
                return redirect()->route('dashboard');
            }
